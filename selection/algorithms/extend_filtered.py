...
def valid_candidate(candidate, cands):
    rep = f"{candidate.table()}({candidate.joined_column_names()})"
    return rep in cands
...
class ExtendFilteredAlgorithm(SelectionAlgorithm):
    def __init__(self, database_connector, parameters=None):
        ...
        self.cands = parse_candidates(self.parameters['candidate_file'])
    ...
    def _calculate_best_indexes(self, workload):
        ...
        while True:
            ...
            for candidate in single_attribute_index_candidates:
                ...
                if candidate not in index_combination and valid_candidate(candidate, self.cands):
        ...
        for position, index in enumerate(index_combination):
            ...
            if index.appendable_by(attribute):
                ...
                if new_index in index_combination:
                    continue
                if not valid_candidate(new_index, self.cands):
                    continue

...
## Usage
Configure the file `example_configs/tpcc_db2.json`. You will need the following:
- Postgres connection information (`database_cargs`)
- Name of database to connect to (`explicit_database_name`)
- DDL Describing the tables in the database (`explicit_ddl_path`)
- Representative in the form of comma-separated sql (`explicit_query_file`)
- List of actions to consider, as generated by the framework (`candidate_file`)

<!-- Install script:
* `./scripts/install.sh` -->

Run index selection evaluation for the TPC-C benchmark:
* `python3 -m selection example_configs/tpcc_db2.json`
